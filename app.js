...
if (!config.SENDGRID_API_KEY) { //used for sendgrid
	throw new Error('missing SENDGRID_API_KEY');
}
if (!config.EMAIL_FROM) { //email from
	throw new Error('missing EMAIL_FROM');
}
if (!config.EMAIL_TO) { //email to
	...
	switch (action) {
		...
		if (isDefined(contexts[0]) && contexts[0].name =='job_application' && contexts[0].parameters) {
			...
			if (phone_number != '' && user_name != '' && previous_job != '' && years_of_experience != '' && job_vacancy != '')
				...
				{
					let emailContent = 'A new short form application from ' + user_name + ' for the job: ' + job_vacancy + '.<br> Previous job position: ' + previous_job + '.' + '.<br> Years of experience: ' + years_of_experience + '.' + '.<br> Phone Number: ' + phone_number + '.' ;
